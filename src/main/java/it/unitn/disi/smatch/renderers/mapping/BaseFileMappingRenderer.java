...
public abstract class BaseFileMappingRenderer extends BaseMappingRenderer {
    ...
    @Override
    protected void process(IContextMapping<INode> mapping, String outputFile, MappingProgressContainer progressContainer) throws MappingRendererException {
        try (BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFile), "UTF-8"))) {
            process(mapping, out, progressContainer);
        } catch (IOException e) {
            throw new MappingRendererException(e.getClass().getSimpleName() + ": " + e.getMessage(), e);
        }
    }

    protected abstract void process(IContextMapping<INode> mapping, BufferedWriter out, MappingProgressContainer progressContainer) throws IOException, MappingRendererException;
